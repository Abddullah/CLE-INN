...
const WeekdayTimeSelector = ({ theme, colors, onSelectedDaysChange, selectedDays }) => {
    ...
    const handleTimeChange = (day, type, time) => {
        ...
        const originalTime = moment(time).format('LLLL')
        const today = moment().format('dddd, MMMM D, YYYY');
        const timePart = moment(originalTime, 'dddd, MMMM D, YYYY h:mm A').format('h:mm A');
        const updatedTime = `${today} ${timePart}`;

        const milliseconds = Number(moment(updatedTime).format('x'));

        const milliseconds1 = time.getHours() * 3600000 + time.getMinutes() * 60000;
            ...
            <View style={styles.column}>
                ...
                >
                    <Text style={{
                        ...
                        {days[day].openingTime == null ? 'Select Time' : moment(days[day].openingTime).format('LT')}
                    </Text>
                </TouchableOpacity>
            </View>
            <View style={styles.column}>
                ...
                >
                    <Text style={{ color: colors.Neutral_01, fontSize: RFValue(12, screenResolution.screenHeight) }}>
                        ...
                        {days[day].closingTime == null ? 'Select Time' : moment(days[day].closingTime).format('LT')}
                    </Text>
                </TouchableOpacity>
            </View>
        </View>
